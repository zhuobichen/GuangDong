...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc 
	...
	IMPLICIT NONE
        ...
        data PNAME / 'emis_ocean' /

        character*46, base
        data base /'/bigdata/emis-make/Youzhiqiang/Forecast/D3/GD/'/

        allocate (Emis(ncol,nrow,1,2,1))
        Emis = 0
...
!        print*, "working on "// SECNAME(n) // " nox"
        status=nf_open(base//"nclproject/oceanfile"
     &  //"/oceanfile.nc",nf_nowrite,ncid)
      if(status==nf_noerr) then
        status=nf_inq_varid(ncid,'OPEN',fid)
        if(status/=nf_noerr) call handle_nc_err(status)
        status=nf_get_var_real(ncid,fid,RawData(:,:))
        if(status/=nf_noerr) call handle_nc_err(status)
            do i=1,ncol
             do j=1,nrow
		Emis(i,j,1,1,1) = RawData(i,j)
	     enddo
            enddo	
        status=nf_close(ncid)
        if(status/=nf_noerr) call handle_nc_err(status)
      else
        print*,"is not available for open"
      endif

! calc for SURF
!        print*, "working on "// SECNAME(n) // GC_SPC2(4)
        status=nf_open(base//"nclproject"
     &  //"/oceanfile/oceanfile.nc",nf_nowrite,ncid)
      if(status==nf_noerr) then
        status=nf_inq_varid(ncid,'SURF',fid)
        if(status/=nf_noerr) call handle_nc_err(status)
        status=nf_get_var_real(ncid,fid,RawData(:,:))
        if(status/=nf_noerr) call handle_nc_err(status)
             do i=1,ncol
              do j=1,nrow
		Emis(i,j,1,2,1) = RawData(i,j) * 0
              enddo
             enddo
        status=nf_close(ncid)
        if(status/=nf_noerr) call handle_nc_err(status)
      else
        print*, "is not available for surf"
      endif
...
!         DO J=1,1
         ...
         JTIME=STIME3D+(1-1)*10000
             ...
             IF ( .NOT. WRITE3( FNAME, GC_SPC2(I),
     $         JDATE, JTIME, Emis(1,1,1,I,1) ) )
     $         CALL M3ERR( PNAME, JDATE, JTIME,
     $        'Could not write '// GC_SPC2(I) //'
     $         to ' // FNAME // ' file',
     $         .TRUE. )
